...
def plot_pr(ckpt_y, ckpt_yhat, ckpt_scores, output_file):
    ...
    enc = OneHotEncoder()
    mu_accs = np.asarray(
        [np.mean(ckpt_y[idx] == ckpt_yhat[idx])
            for idx in range(len(ckpt_y))])
    argmax_acc = np.argmax(mu_accs)
    max_acc = np.max(mu_accs)
    p, r, mu_p = precision_recall(
        enc.fit_transform(ckpt_y[argmax_acc].reshape(-1, 1))
        .todense().reshape(-1, 1), ckpt_scores[argmax_acc].reshape(-1, 1))
    # Plot Precision-Recall curve
    plt.figure()
    plt.plot(
        r, p, lw=2, label='Precision-Recall curve for checkpoint ' +
...
def plot_cms(ckpt_y, ckpt_yhat, config, output_file, normalize=True):
    ...
    plt.grid(False)
    plt.title(
        config.which_dataset + ' confusion matrix; ' + str(max_acc * 100)
        + '%')
    plt.colorbar()
...
def plot_image_hm(im, hm, target_file, title=None):
    fig = plt.figure()
    plt.subplot(1, 2, 1)
    plt.imshow(im)
    plt.subplot(1, 2, 2)
