...
def read_and_decode(
    ...
    image = tf.reshape(image, im_size)
    if heatmap is not None:
        heatmap = tf.reshape(heatmap, [im_size[0], im_size[1], 1])

    # Preprocess images and heatmaps
    image = tf.expand_dims(image, axis=0)  # Add singleton for augmentation
    if heatmap is not None:
        ...
        heatmap = tf.expand_dims(heatmap, axis=0)  # Add singleton for aug
        ...
        heatmap = heatmap[0]  # Squeeze off the first dimension
    else:
        image = image_augmentations(
            ...
            model_input_shape,
            return_heatmaps)
    image = image[0]  # Squeeze off the first dimension
...
def inputs(
    ...
    enqueue_many = False  # Keep this False.
    with tf.name_scope('input'):
        ...
        batch_data = read_and_decode(
            ...
            return_heatmaps=return_heatmaps,
            return_clicks=return_clicks,
            batch_size=1)  # Set to 1 for single-exemplar loading
